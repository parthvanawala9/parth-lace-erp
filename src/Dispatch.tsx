...
export default function Dispatch() {
  ...
  const [historyViewGroup, setHistoryViewGroup] = useState<any>(null);
                        ...
                        <td className="py-3.5 px-4">
                          <div className="flex justify-center gap-1.5">
                            <button
                              ...
                              className="inline-flex items-center justify-center px-2.5 py-1.5 text-xs font-semibold text-slate-700 bg-slate-100 border border-slate-200 rounded-lg hover:bg-slate-200 transition-colors gap-1"
                            >
                              <Eye className="w-3.5 h-3.5 text-slate-500" />
                              View
                            </button>
                            <button
                              ...
                              className="inline-flex items-center justify-center px-2.5 py-1.5 text-xs font-semibold text-slate-700 bg-white border border-slate-300 rounded-lg hover:bg-slate-50 transition-colors shadow-sm gap-1.5"
                            >
                ...
                {groupedHistoryItems.map((group) => (
                  <div key={group.groupKey} className="p-4 space-y-3 bg-white">
                    ...
                    <div className="pt-1 flex justify-end gap-1.5">
                      <button
                        onClick={() => setHistoryViewGroup(group)}
                        className="inline-flex items-center justify-center px-3 py-1.5 text-xs font-semibold text-slate-700 bg-slate-100 border border-slate-200 rounded-lg hover:bg-slate-200 transition-colors gap-1"
                      >
                        <Eye className="w-3.5 h-3.5 text-slate-500" />
                        View
                      </button>
          ...
          <div className="bg-white rounded-xl shadow-xl max-w-lg w-full p-6 space-y-4 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center border-b border-slate-100 pb-3">
              <div>
                ...
                <p className="text-xs text-slate-500 font-mono mt-0.5">
                  Order #{historyViewGroup.orderNo} • Party: {historyViewGroup.partyName}
                </p>
              </div>
              <button
                onClick={() => setHistoryViewGroup(null)}
                className="text-slate-400 hover:text-slate-600 text-sm font-bold"
              >
                ✕
              </button>
            ...
            <div className="grid grid-cols-2 gap-3 text-sm bg-slate-50 p-3 rounded-lg border border-slate-200">
              <div>
                <span className="text-[10px] text-slate-400 uppercase font-semibold block">Dispatch Date</span>
                <span className="font-semibold text-slate-800">{historyViewGroup.dispatchDate || "-"}</span>
              </div>
              <div>
                <span className="text-[10px] text-slate-400 uppercase font-semibold block">Status</span>
                <div className="mt-1">{renderStatusBadge(historyViewGroup.status)}</div>
              </div>
              <div>
                <span className="text-[10px] text-slate-400 uppercase font-semibold block">Designs</span>
                <span className="font-semibold text-slate-800">{historyViewGroup.designs.join(", ") || "-"}</span>
              </div>
              <div>
                <span className="text-[10px] text-slate-400 uppercase font-semibold block">Dispatched Qty</span>
                <span className="font-bold text-slate-900">
                  {historyViewGroup.totalDispatched} {historyViewGroup.unit}
                </span>
              </div>
            </div>

            <div className="border border-slate-200 rounded-lg overflow-hidden">
              <div className="px-3 py-2 bg-slate-100 text-xs font-bold text-slate-700 uppercase">
                Colours
              </div>
              <div className="p-3 flex flex-wrap gap-1.5">
                {historyViewGroup.colours.length > 0 ? (
                  historyViewGroup.colours.map((colour: string, i: number) => (
                    <span
                      key={i}
                      className="inline-flex items-center px-2 py-1 rounded text-[11px] font-medium bg-slate-100 text-slate-800 border border-slate-200"
                    >
                      {colour}
                    </span>
                  ))
                ) : (
                  <span className="text-xs text-slate-500">No colours recorded</span>
                )}
              </div>
            </div>
            ...
            <div className="border border-slate-200 rounded-lg overflow-hidden">
              ...
              <div className="divide-y divide-slate-100">
                {historyViewGroup.logs.map((log: DispatchRecord, i: number) => {
                  const qty = Number(
                    log.dispatch_qty ??
                    log.dispatched_qty ??
                    log.qty ??
                  ...
                  const colour = log.order_items?.colours?.colour_name || "-";
                  const design = log.order_items?.designs?.design_name || "-";
                  return (
                    <div key={log.id || i} className="px-3 py-2.5 flex items-center justify-between gap-3 text-xs">
                      <div>
                        <div className="font-semibold text-slate-800">{design}</div>
                        <div className="text-slate-500">{colour}</div>
                      </div>
                      <div className="font-bold text-slate-900 whitespace-nowrap">
                        {qty} {log.order_items?.unit || historyViewGroup.unit}
                      </div>
                    </div>
                  );
                })}
